...
function TouchMove(e) {
  if (e.touches.length > 0) {
    ...
    if (withinBounds) e.preventDefault()
  }
}
...
class Z extends d {
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0) {
        U.scale.setScalar(0)
        if (this.config.followCursor === false) {
          U.position.set(0, -9999, 0)
        }
      } else {
      ...
      if (idx === 0 && this.config.followCursor !== false) this.light.position.copy(U.position)
    }
...
function createBallpit(e, t = {}) {
  ...
  const h = S({
    domElement: e,
    onMove() {
      if (s.config.followCursor === false) {
